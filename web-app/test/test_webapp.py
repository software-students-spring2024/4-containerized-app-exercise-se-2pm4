...
class Tests:
    ...
    def test_home_post(client):
        ...
        """
        test_img_path = "/Users/yiweiluo/4-containerized-app-exercise-se-2pm4/web-app/img/man.jpg"
        with open(test_img_path, 'rb') as image_file:
            image_data = image_file.read()
        response = client.post('/', data = {'image': (image_data, 'man.jpg')}, content_type='multipart/form-data')
        assert response.status_code == 200
    @pytest.fixture
    ...
    def test_check_status(client):
        sample_id = 'sample_id'
        images_collection.insert_one({"_id": sample_id, "processed": False, "emotion": "happy"})
